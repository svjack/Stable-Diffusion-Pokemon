...
args_dict["caption_column"] = "ja_text"
...
dataset_name_mapping = {
    "svjack/pokemon-blip-captions-en-ja": ("image", "ja_text"),
}
...
pipeline.save_pretrained("jap_model_50000")
pipeline.safety_checker = lambda images, clip_input: (images, False)
pipeline = pipeline.to("cuda")

imgs = pipeline("鉢植えの植物を頭に載せた漫画のキャラクター",
...
imgs = pipeline("黄色いボール",
                    num_inference_steps = 100
)
imgs.images[0]

imgs = pipeline("ブルードラゴンのイラスト",
                    num_inference_steps = 100
)
imgs.images[0]
