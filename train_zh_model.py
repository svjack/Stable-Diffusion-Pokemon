...
args_dict["caption_column"] = "zh_text"
...
tokenizer = BertTokenizer.from_pretrained("IDEA-CCNL/Taiyi-CLIP-Roberta-102M-Chinese")
text_encoder = BertForTokenClassification.from_pretrained("IDEA-CCNL/Taiyi-CLIP-Roberta-102M-Chinese")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                eh_shape = encoder_hidden_states.shape
                eh_pad = torch.zeros((eh_shape[0], eh_shape[1], 768 - 512))
                eh_pad = eh_pad.to(accelerator.device)
                encoder_hidden_states = torch.concat([encoder_hidden_states, eh_pad], -1)
